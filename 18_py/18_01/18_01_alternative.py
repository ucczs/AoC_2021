...
result = ()
maxMagnitude = 0
for secondLineIdx in range(len(lines)):
    if secondLineIdx == 0:
        continue

    for firstLineIdx in range(secondLineIdx-1):
        for i in range(2):
            if i == 0:
                secondLine = list(lines[secondLineIdx].strip())
                firstLine = list(lines[firstLineIdx].strip())
            else:
                secondLine = list(lines[firstLineIdx].strip())
                firstLine = list(lines[secondLineIdx].strip())
            ...
            newMagnitude = calcMagnitude(mergedLines)
